...
BOOST_AUTO_TEST_SUITE(FilterTest)
   ...
   BOOST_AUTO_TEST_CASE(CheckIfDefaultValueIsZero) {
	BOOST_CHECK_EQUAL(Filter().get(), 0.0f);
    }

   BOOST_AUTO_TEST_CASE(CheckIfUpdateValueWorks) {
	BOOST_CHECK_CLOSE(Filter().update(2.0), 1.6f, 0.0001);
	BOOST_CHECK_CLOSE(Filter().update(-1.0f), -0.8f, 0.0001);
    }

   BOOST_AUTO_TEST_CASE(CheckIfUAssignsProperlyNewFilterValue) {
	   Filter filter;
	   filter = 2.0;
	   BOOST_CHECK_CLOSE(filter.get(), 1.6f, 0.0001);
   }
